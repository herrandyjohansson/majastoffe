  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[var(--background)]/90 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5 lg:px-8">
        ...
        <nav className="hidden items-center gap-8 lg:flex">
          {sections.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="text-[11px] font-medium uppercase tracking-[0.15em] text-[var(--foreground)]/70 transition-colors hover:text-[var(--foreground)]"
            >
              {item.label}
            </a>
          ))}
        ...
        <button
          type="button"
          className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.15em] text-[var(--foreground)] lg:hidden"
